...
module.exports = function loadSortedPlugins (globalDirectory, externalRessourcesDirectory, files, loadedPlugins, orderedPluginsNames, emit, logger, data) {

	// if no files, does not run
	return !files.length ? Promise.resolve() : Promise.resolve().then(() => {

		const sortedPluginsNames = [ ...files.filter((pluginName) => {
