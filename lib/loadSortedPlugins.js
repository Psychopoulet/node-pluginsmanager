...
// private
	...
	// methods
		...
		* @param {string} globalDirectory : get plugins directory
		* @param {string} externalRessourcesDirectory : get ressources directory
		* @param {string} toLoad : order for load
		* @param {object} plugins : already loaded plugins
		* @param {function} emit : emit data function
		* @param {object} data : data to send
		...
		* @return {Promise} : result operation
		*/
		function _loadSortedPlugins (globalDirectory, externalRessourcesDirectory, toLoad, plugins, emit, data, i = 0) {

			return i < toLoad.length ? Promise.resolve().then(() => {

				return _loadPlugin(globalDirectory, externalRessourcesDirectory, toLoad[i], plugins, emit, data);

			// loop
			}).then(() => {

				return _loadSortedPlugins(globalDirectory, externalRessourcesDirectory, toLoad, plugins, emit, data, i + 1);
		...
		* @return {Promise} : result operation
		*/
		function _loadUnsortedPlugins (globalDirectory, externalRessourcesDirectory, toLoad, plugins, emit, data, i = 0) {
			...
			// loop
				...
				return _loadUnsortedPlugins(globalDirectory, externalRessourcesDirectory, toLoad, plugins, emit, data, i + 1);
...
module.exports = (globalDirectory, externalRessourcesDirectory, files, plugins, orderedPluginsNames, emit, data) => {
	...
	return !files.length ? Promise.resolve() : Promise.resolve().then(() => {
		...
		return _loadSortedPlugins(globalDirectory, externalRessourcesDirectory, files.filter((pluginName) => {
			return orderedPluginsNames.includes(pluginName);
		}), plugins, emit, data);

	}).then(() => {

		return _loadUnsortedPlugins(globalDirectory, externalRessourcesDirectory, files.filter((pluginName) => {
			return !orderedPluginsNames.includes(pluginName);
		}), plugins, emit, data);
