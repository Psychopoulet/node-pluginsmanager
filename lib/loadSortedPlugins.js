...
// private
	...
	// methods
		...
		* @return {Promise} : result operation
		*/
		function _loadPlugins (globalDirectory, externalRessourcesDirectory, toLoad, plugins, emit, data, i = 0) {
			...
			// loop
				...
				return _loadPlugins(globalDirectory, externalRessourcesDirectory, toLoad, plugins, emit, data, i + 1);
...
module.exports = (globalDirectory, externalRessourcesDirectory, files, plugins, orderedPluginsNames, emit, data) => {
	...
	return !files.length ? Promise.resolve() : Promise.resolve().then(() => {
		...
		return _loadPlugins(globalDirectory, externalRessourcesDirectory, orderedPluginsNames, plugins, emit, data);
