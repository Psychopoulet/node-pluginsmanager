	...
	// public
		...
		// load / destroy
			...
			loadAll (data) {
				...
				return isAbsoluteDirectory("loadAll/directory", this.directory).then(() => {
				...
				// load all
					...
					const _loadPlugin = (i = 0) => {
						...
						return i < files.length ? Promise.resolve().then(() => {
							...
							// is already exists ?
							return plugin ? Promise.resolve(plugin) : createPluginByDirectory(directory, data, this.externalRessourcesDirectory);
