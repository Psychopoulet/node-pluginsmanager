		...
		// init / release
			...
			initAll (data) {
				...
				// init plugins
					...
					const _initPlugin = (i = 0) => {

						return i < this.plugins.length ? Promise.resolve().then(() => {

							return this.plugins[i].init(data);

						// emit event
						}).then(() => {

							this.emit("initialized", this.plugins[i]);
