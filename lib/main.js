...
// deps
	...
	const npm = require(path.join(__dirname, "npm.js"));
	const Plugin = require(path.join(__dirname, "plugin.js"));
	const stdToString = require(path.join(__dirname, "stdToString.js"));
	...
	// private
		...
		*/
		_error (fnName, err) {
			...
			this.emit("error", fnName + " : " + stdToString(err));
					...
					return Promise.resolve().then(() => {
						...
						return fs.isDirectoryProm(directory).then((exists) => {
							...
							// install dependencies & execute install script
								...
								(0, console).log(plugin);

								return Promise.resolve().then(() => {
									return !plugin.dependencies ? Promise.resolve() : npm(directory, "install --prod");
								}).then(() => {
			...
			updateByKey (key, data) {
				...
				return Promise.resolve().then(() => {
					...
					return plugin.unload(data, false).then(() => {
						...
						// download plugin
						return !plugin.github ? Promise.resolve() : git([ "-c", plugin.directory, "pull" ]);
						...
						return Promise.resolve().then(() => {
							return !_plugin.dependencies ? Promise.resolve() : npm(_plugin.directory, "update --prod");
						}).then(() => {
