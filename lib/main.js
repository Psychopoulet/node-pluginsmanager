			...
			*/
			function _GIT (params) {
				...
				return Promise.resolve().then(() => {
					...
					return -1 >= process.env.PATH.indexOf("Git") && -1 >= process.env.PATH.indexOf("git") ?
							Promise.reject(new Error("\"git\" is probably not registered in your PATH")) :
							Promise.resolve();
			...
			*/
			function _NPM (directory, command) {
				...
				return Promise.resolve().then(() => {
					...
					return -1 >= process.env.PATH.indexOf("npm") ?
							Promise.reject(new Error("\"npm\" is probably not registered in your PATH")) :
							Promise.resolve();
		...
		setOrder (pluginsDirectoriesBaseNames) {
			...
			return Promise.resolve().then(() => {
				...
				return "object" !== typeof pluginsDirectoriesBaseNames || !(pluginsDirectoriesBaseNames instanceof Array) ?
						_error(this, "setOrder", "This is not an array") :
						Promise.resolve();
				...
				return !errors.length ? Promise.resolve() : _error(this, "setOrder", errors.join("\r\n"));
		...
		beforeLoadAll (callback) {
			...
			return Promise.resolve().then(() => {
				...
				return "function" !== typeof callback ?
						_error(this, "beforeLoadAll", "This is not a function") :
						Promise.resolve();
		...
		loadByDirectory (directory, data) {
			...
			return Promise.resolve().then(() => {
				...
				return "string" !== typeof directory ?
						_error(this, "loadByDirectory", "\"directory\" parameter is not a string") :
						Promise.resolve();
				...
				return plugin ? Promise.resolve(plugin) : fs.isDirectoryProm(directory).then((exists) => {
					...
					return !exists ? Promise.resolve() : _createPluginByDirectory(directory).then((_plugin) => {
		...
		loadAll (data) {
			...
			return Promise.resolve().then(() => {
				...
				return "" === this.directory ?
						_error(this, "loadAll", "\"directory\" is not defined") :
						Promise.resolve();
		...
		unloadByKey (key, data) {
			...
			return Promise.resolve().then(() => {
				...
				return "number" !== typeof key ?
						_error(this, "unloadByKey", "\"key\" is not an integer") :
						Promise.resolve();

			}).then(() => {

				return !this.plugins[key] ?
						_error(this, "unloadByKey", "There is no \"" + key + "\" plugin's key") :
						Promise.resolve();
			...
			unloadByDirectory (directory, data) {
				...
				return fs.isDirectoryProm(directory).then((exists) => {
					...
					return Promise.resolve().then(() => {
						...
						return !exists ?
								_error(this, "unloadByDirectory", "There is no \"" + directory + "\" plugin's directory") :
								Promise.resolve();
		...
		installViaGithub (url, data) {
			...
			return Promise.resolve().then(() => {
				...
				return "string" !== typeof url ?
						_error(this, "installViaGithub", "\"url\" is not a string") :
						Promise.resolve();
