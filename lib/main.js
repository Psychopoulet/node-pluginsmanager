					...
					return fs.isDirectoryProm(directory).then((exists) => {
						...
						if (!exists) {
							...
							return _createPluginByDirectory(directory).then((_plugin) => {
								...
								return _error(this, "loadByDirectory", "\"" + ((plugin) ? plugin.name : path.basename(directory)) + "\" => " + ((err.message) ? err.message : err));
							});
		...
		updateByKey (key, data) {
			...
			if ("number" !== typeof key) {
				...
				return plugin.unload((data) ? data : null, false).then(() => {
					...
					let updateFile = path.join(plugin.directory, "package-lock.json");
