	...
	constructor (options) {
		...
		// public
			...
			this.directory = options && "undefined" !== typeof options.directory ?
				options.directory :
				join(homedir(), "node-pluginsmanager-plugins");
			...
			this.externalRessourcesDirectory = options && "undefined" !== typeof options.externalRessourcesDirectory ?
				...
				join(homedir(), "node-pluginsmanager-plugins-ressources");
	...
	// public
		...
		// network
			...
			appMiddleware (req, res, next) {
				...
				for (let i = 0; i < this.plugins.length; ++i) {
					this.plugins[i].appMiddleware(req, res, next);
				}
			...
			httpMiddleware (req, res) {
				...
				for (let i = 0; i < this.plugins.length; ++i) {
					...
					if (this.plugins[i].httpMiddleware(req, res)) {
						return true;
					}
		...
		// load / destroy
			...
			loadAll (data) {
				...
				return isNonEmptyString("initAll/directory", this.directory).then(() => {
					...
					return isNonEmptyString("initAll/externalRessourcesDirectory", this.externalRessourcesDirectory);
				...
				// load all
					...
					const _loadPlugin = (i = 0) => {

						return i < files.length ? Promise.resolve().then(() => {

							const directory = join(this.directory, files[i]);
							...
							// is already exists ?
							return plugin ? Promise.resolve(plugin) : createPluginByDirectory(directory, this.externalRessourcesDirectory, data);
		...
		// init / release
			...
			initAll (data) {
				...
				// create dir if not exist
					...
					return isNonEmptyString("initAll/externalRessourcesDirectory", this.externalRessourcesDirectory).then(() => {
						...
						return mkdirpProm(this.externalRessourcesDirectory);
				...
				// init plugins
					...
					return Promise.resolve().then(() => {
						...
						const orderedPlugins = this.plugins.filter((plugin) => {
							return -1 < this._orderedPluginsNames.indexOf(plugin.name);
						});

						const _initOrderedPlugin = (i = 0) => {

							return i < orderedPlugins.length ? Promise.resolve().then(() => {

								return orderedPlugins[i].init(data);

							// emit event
							}).then(() => {

								this.emit("initialized", orderedPlugins[i]);
							...
							// loop
								...
								return _initOrderedPlugin(i + 1);

							}) : Promise.resolve();

						};

						return _initOrderedPlugin();

					// others (async)
					}).then(() => {

						const notOrderedPlugins = this.plugins.filter((plugin) => {
							return -1 >= this._orderedPluginsNames.indexOf(plugin.name);
						});

						return notOrderedPlugins.length ? new Promise((resolve, reject) => {
			...
			releaseAll (data) {
				...
				return Promise.resolve().then(() => {
					...
					const _releasePlugin = (i = this.plugins.length - 1) => {
						...
						// emit event
							...
							this.emit("released", this.plugins[i]);
				...
				// remove all external ressources
				}).then(() => {
			...
			installViaGithub (user, repo, data) {
				...
				return isAbsoluteDirectory("installViaGithub/directory", this.directory).then(() => {
					...
					return Promise.resolve().then(() => {
						...
						return createPluginByDirectory(directory, this.externalRessourcesDirectory);
