		...
		*/
		_loadOrderedNext (i, data) {
			...
			if (i >= this._orderedDirectoriesBaseNames.length) {
				...
				return this.loadByDirectory(path.join(this.directory, this._orderedDirectoriesBaseNames[i]), data).then(() => {
			...
			loadByDirectory (directory, data) {
				...
				return Promise.resolve().then(() => {
					...
					return plugin ? Promise.resolve(plugin) : fs.isDirectoryProm(directory).then((exists) => {
						...
						return !exists ?
							Promise.resolve() : this._createPluginByDirectory(directory).then((_plugin) => {
