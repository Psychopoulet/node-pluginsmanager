	...
	// methods
		...
		function _unloadNext(that, data) {
			...
			if (0 >= that.plugins.length) {
				...
				let i = that.plugins.length - 1;
				return that.plugins[i].unload((data) ? data : null).then(() => {
					that.plugins.splice(i, 1);
					return Promise.resolve();
				});
			...
			if ("number" !== typeof key) {
				...
				return plugin.unload((data) ? data : null, false).then(() => {
					...
					return this.uninstallByDirectory(plugin.directory, (data) ? data : null).then(() => {
						return Promise.reject("Impossible to update \"" + plugin.directory + "\" directory : " + err);
					});
