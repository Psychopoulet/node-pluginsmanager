		...
		// network
				...
				if (!this.plugins.length) {
					...
					const _recursiveNext = (i) => {
						...
						if (i >= this.plugins.length) {
							...
							return () => {
								return this.plugins[i].appMiddleware(req, res, _recursiveNext(i + 1));
							};
					...
					return this.plugins[0].appMiddleware(req, res, _recursiveNext(1));
			...
			socketMiddleware (server) {
				...
				for (let i = 0; i < this.plugins.length; ++i) {
					this.plugins[i].socketMiddleware(server);
				}
