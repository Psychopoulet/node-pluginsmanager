...
// private
	...
	// methods
		...
		function _spawnProm(command, args, options) {

			return new Promise((resolve, reject) => {

				let result = "", mySpawn = spawn(command, args, options).on("error", (msg) => {
			...
			function _NPMUpdate(directory) {
				...
				if (-1 >= process.env.PATH.indexOf("npm")) {
					...
					return _spawnProm((("win32" === process.platform) ? "npm.cmd" : "npm"), [ "-v" ], { cwd: directory });
				}
	...
	// write
					...
					return fs.isDirectoryProm(directory).then((exists) => {
						...
						if (exists) {
							...
							return _GITClone(url, directory).then(() => {
								...
								return _NPMUpdate(directory).then(() => {
		...
		updateByKey (key, data) {
			...
			if ("number" !== typeof key) {
				...
				let directory = this.plugins[key].directory;
				...
				return this.plugins[key].unload((data) ? data : null, false).then(() => {
					...
					if (!this.plugins[key].github) {
						return Promise.resolve();
					}
					else {
						return _GITUpdate(directory);
					}
					...
					return _NPMUpdate(plugin.directory).then(() => {
					...
					return this.uninstallByDirectory(directory, (data) ? data : null).then(() => {
						return Promise.reject("Impossible to update \"" + directory + "\" directory : " + err);
					});
