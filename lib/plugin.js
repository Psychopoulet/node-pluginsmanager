	...
	loadDataFromPackageFile () {
		...
		try {
			...
			return fs.isDirectoryProm(this.directory).then(function(exists) {
				...
				if (!exists) {
					return Promise.reject("PluginManager/Plugin/loadDataFromPackageFile : \"" + that.directory + "\" does not exist.");
				}
				else {
					...
					return fs.isFileProm(file).then(function(exists) {
						...
						if (!exists) {
							return Promise.reject("PluginManager/Plugin/loadDataFromPackageFile : \"" + file + "\" does not exist.");
						}
						else {
